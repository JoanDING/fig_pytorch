...
def train(opt):
  ...
  for ep_id in range(opt.max_eps):
    ...
    for iter_id, train_batch in enumerate(data_loader_train):
      ...
      alphas1 = model.alpha1
      alphas2 = model.alpha2
    t1 = time.time()
...
def main():
  ...
  parser.add_argument('--att_rec', default=1, type=int,
                      help='if use recurrent attention')
  # general settings -----------------------------------------------------------------
  ...
  parser.add_argument('--weight_decay2', default=5e-5, type=float,
                      help='weight_decay2')
  parser.add_argument('--weight_decay_att', default=5e-5, type=float,
