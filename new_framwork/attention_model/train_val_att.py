...
def train(opt):
  ...
  elif opt.dataset == 'frappe':
    data_train = FrappeTrain(opt.data_path)
    data_test = FrappeTest(opt.data_path)
...
def main():
  ...
  parser.add_argument('--type', default=10160, type=int,
  ...
  if opt.dataset == 'men':
      ...
      opt.num_node = 25
      opt.data_path = '/storage/yjding/djj_mask/pog-full'
  elif opt.dataset == 'frappe':
      opt.num_node = 9
      opt.data_path = '/storage/yjding/djj_mask/frappe'
