...
def train(opt):
  ...
  opt.vocab_size = sum(data_train.cnt[:2])
  opt.self_att = False
  ...
  for ep_id in range(opt.max_eps):
    ...
    if ep_id%eval_freq==0:
      ...
      for test_batch in data_loader_test:
        ...
        _, sort_idx = torch.sort(scores[:,0,0],descending=True)
...
def main():
  ...
  parser.add_argument('--lr_em', default=0.0002, type=float,
                      help='initial learning rate')
