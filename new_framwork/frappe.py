...
def prepare_data(in_path):
  ...
  train_file = 'frappe.train'
  test_file = 'frappe.test'
  ...
  test_neg_file = os.path.join(in_path, 'frappe.test.negatives_by')
  ...
  with open(os.path.join(in_path, 'frappe.indice_map.json'),'w') as f:
...
class FrappeTrain(data.Dataset):
  def __init__(self, in_path, split='train'):
    super(FrappeTrain, self).__init__()
    data_file = os.path.join(in_path, '{}_data.json'.format(split))
    indice_map_file = os.path.join(in_path, 'frappe.indice_map.json')
...
class FrappeTest(data.Dataset):
  def __init__(self, in_path, split='test'):
    super(FrappeTest, self).__init__()
