...
def prepare_data(in_path):
  ...
  train_file = 'pog-full.train'
  test_file = 'pog-full.test'
  ...
  test_neg_file = os.path.join(in_path, 'pog-full.test.negatives_by')
  ...
  with open(os.path.join(in_path, 'pog-full.indice_map.json'),'w') as f:
...
class PogfullTrain(data.Dataset):
  def __init__(self, in_path, split='train'):
    super(PogfullTrain, self).__init__()
    data_file = os.path.join(in_path, '{}_data.json'.format(split))
    indice_map_file = os.path.join(in_path, 'pog-full.indice_map.json')
...
class PogfullTest(data.Dataset):
  def __init__(self, in_path, split='test'):
    super(PogfullTest, self).__init__()
