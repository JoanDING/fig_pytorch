...
def prepare_data(in_path):
  ...
  train_file = 'amazon-women-group.train'
  test_file = 'amazon-women-group.test'
  ...
  for i in range(num_node):
    ...
    cnt.append(num_ids)
    id_map = {}
    for idx, idd in enumerate(ids_set):
      id_map[idd] = idx + begin_idx
  ...
  test_neg_file = os.path.join(in_path, 'amazon-women-group.test.negatives_by')
  ...
  with open(os.path.join(in_path, 'amazon-women-group.indice_map.json'),'w') as f:
...
class WomenTrain(data.Dataset):
  def __init__(self, in_path, split='train'):
    import pdb
    super(WomenTrain, self).__init__()
    data_file = os.path.join(in_path, '{}_data.json'.format(split))
    indice_map_file = os.path.join(in_path, 'amazon-women-group.indice_map.json')
...
class WomenTest(data.Dataset):
  def __init__(self, in_path, split='test'):
    super(WomenTest, self).__init__()
  ...
  def __len__(self):
    ...
    # print('preparing data...')
    # prepare_data('/home/binyi/amazon-women-group-cp')
